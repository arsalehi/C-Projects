...
bool bf_probe(BloomFilter *bf, char *key) {
  uint8_t bit1 =
      bv_get_bit(bf->filter, hash(bf->primary, key) % bv_get_len(bf->filter));
  uint8_t bit2 =
      bv_get_bit(bf->filter, hash(bf->secondary, key) % bv_get_len(bf->filter));
  uint8_t bit3 =
      bv_get_bit(bf->filter, hash(bf->tertiary, key) % bv_get_len(bf->filter));
