...
double Sin(void){
    ...
    for (double x = -2 * M_PI; x < 2* M_PI; x+= (M_PI/16)){
        ...
        num = x_norm * ( ( x_squared * ( 52785432 - (479249 * x_squared)) - 1640635920) * x_squared + 11511339840);
        den = (((18361 * x_squared + 3177720) * x_squared + 277920720) * x_squared + 11511339840);
        sin_x = num / den;
        
        // Print output
        //printf("% 6.4lf \t% 10.8lf \t% 10.8lf \t% 12.10lf\n", x, sin_x, sin(x), sin(x) - sin_x);
        printf("% 6.4lf\t% 10.8lf\t% 10.8lf\t% 12.10lf\n", x, sin_x, sin(x), sin(x) - sin_x);
...
double Cos(void){
    ...
    for (double x = -2 * M_PI; x < 2* M_PI; x+= (M_PI/16)){
        ...
        num = ((x_squared * (1075032 - 14615 * x_squared) - 18471600) * x_squared + 39251520);
        den = ((( 127 * x_squared + 16632) * x_squared + 1154160) * x_squared + 39251520);
        cos_x = num / den;

        //Print output
        //printf("% 6.4lf \t% 10.8lf \t% 10.8lf \t% 12.10lf\n", x, cos_x, cos(x), cos(x) - cos_x);
        printf("% 6.4lf\t% 10.8lf\t% 10.8lf\t% 12.10lf\n", x, cos_x, cos(x), cos(x) - cos_x);
...
double Tan(void){
    ...
    for (double x = (M_PI/ -3); x < (M_PI/3); x+= (M_PI / 16)){
        ...
        //Perform Padé approximation
        num = x * ((((x_squared - 990) * x_squared + 135135)* x_squared - 4729725) * x_squared + 34459425);
        den = 45 * ((((x_squared - 308) * x_squared + 21021) * x_squared - 360360) * x_squared + 765765);
        tan_x = num / den ;

        //Print output
        //printf("% 6.4lf \t% 10.8lf \t% 10.8lf \t% 12.10lf\n", x, tan_x, tan(x), tan(x) - tan_x);
        printf("% 6.4lf\t% 10.8lf\t% 10.8lf\t% 12.10lf\n", x, tan_x, tan(x), tan(x) - tan_x);
...
double Exp(void){
    ...
    for (long double x = 0; x < 9.1 ; x += 0.1){
        ...
        long double sum = prev; //Starts as prev, then increments as prev * x/n is added to form (x^n)/n!

        //Run Taylor approximation until prev converges/ becomes smaller than Epsilon (1e-9)
