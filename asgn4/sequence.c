...
// Produces prime results, printing all prime numbers and classifying special
// primes
...
void primes(BitVector *v) {
  ...
  for (uint32_t i = 2; i <= bv_get_len(v); ++i) {
    ...
    // Used so Lucas numbers begin as 3 and with the two prevous numbers as 2
    // and 1
    if (i == 2) {
      printf(", lucas");
    }
...
char *base_change(uint32_t num, uint32_t base) {
  ...
  while (num > 0) {
    ...
    if (rem < 10) {
      ...
      // Capital W is 10 positions behind lowercase a, so this accounts for
      // rollover
      rem_char = rem + 'W';
    }
