...
Stack* move_smallest(Stack *l, Stack *a, Stack *b, Stack *c){
    ...
    if(stack_empty(potential1)){
        ...
        print_state_s(potential1->items[stack_peek(potential1) -1], potential2->name, potential1->name);
        return potential1;
    } else if (stack_empty(potential2)){
        //Move disk from potential1 to potential2
        temp = stack_pop(potential1);
        stack_push(potential2, temp); 
        print_state_s(potential2->items[stack_peek(potential2) -1], potential1->name, potential2->name);
        return potential2;
    } else if(potential1->items[stack_peek(potential1)-1] < potential2->items[stack_peek(potential2)-1]){
        //Move disk from potential1 to potential2
        temp = stack_pop(potential1);
        stack_push(potential2, temp);
        print_state_s(potential2->items[stack_peek(potential2) -1], potential1->name, potential2->name);
...
//Determine which peg has the smallest disk on top, and which disk was not just added to
...
//Move smallest based on whether last disk moved was even or odd
    switch(l->items[stack_peek(l)-1] % 2){
        ...
        //Move smallest disk to peg NOT last added to 
            ...
            print_state_s(not_last->items[stack_peek(not_last) -1], peg1->name, not_last->name);
            return not_last;
            break;
    }
