...
//
// Reads in a sizeof(FileHeader) bytes from the input file.
...
// In realitly, a block of symbols is read into a buffer.
...
//
// Buffers a pair. A pair is comprised ofa symbol and an index.
// The bits of the index are buffered first, starting from the LSB.
// The bits of the code are buffered next, also starting from the LSB.
...
//
// outfile: File descriptor of the output file to write out.
// sym:     Symbol of the pair to buffer.
// index:   Index of the pair to buffer.
// bit_len: Number of bits of hte index to buffer.
// returns: Void.
//
void buffer_pair(int outfile, uint16_t code, uint8_t sym, uint8_t bitlen);

//
// Writes out any remaining pair of symbols and indexes to the output file.
//
// outfile: File descriptor of the output file to write to.
// returns: Void.
//
...
//
// "Reads" a pair (symbol and indes) from the input file.
// The "read" symbol is placed in the pointer to the sym (pass by reference)
// The "read" index is placed in the pointer to index (pass by reference)
// In reality, a block of pairs read into a buffer.
...
// The next bit_len bits constitutes the index, starting from the LSB.
...
// bit_len: Length in bits of the index to read (POTENTIALLY NEXT IND AVAILABE?)
